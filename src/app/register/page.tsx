...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setUser({
        id: user.uid,
        name: form.name || user.email?.split('@')[0] || "Học viên",
        email: user.email || "",
        xp: 0, level: 1, streakDays: 0, hearts: 5, role: "USER",
      });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4"
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        <Link href="/" className="flex items-center gap-2 text-white mb-6 hover:opacity-80 transition-opacity">
        ...
        <div className="card p-8">
          ...
          <div className="flex flex-col gap-3 mb-4">
            ...
            <button onClick={async () => {
              ...
              className="w-full py-3 rounded-2xl font-bold bg-white border-2 border-gray-200 text-gray-700 transition-all hover:bg-gray-50 flex items-center justify-center gap-2 shadow-sm">
          ...
          <p className="text-center text-sm mt-6" style={{ color: "var(--text-muted)" }}>
            Đã có tài khoản?{" "}
            <Link href="/login" className="font-bold" style={{ color: "var(--green)" }}>Đăng nhập</Link>
          </p>
        </div>
      </motion.div>
    </div>
